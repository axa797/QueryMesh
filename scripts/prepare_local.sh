...
echo "  uv sync && uv run alembic upgrade head"
echo "  PYTHONPATH=. uv run python scripts/mint_api_key.py"
echo "  PYTHONPATH=. uv run uvicorn api.main:app --reload --host 0.0.0.0 --port 8000"
