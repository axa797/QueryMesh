...
- **Langfuse (Phase 13+):** Set `LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, optional `LANGFUSE_HOST` and **`LANGFUSE_TRACING_ENVIRONMENT`** in [.env.example](.env.example) / [api/settings.py](api/settings.py). Graph invoke metadata includes **`user_id`** (internal UUID string) for trace correlation. Traces use the LangChain `CallbackHandler` on each `/query` graph run; response `trace_id` is the root trace id (UUID hex); `flush()` runs after each request. Cloud Monitoring metric names and alert thresholds (stubs) live in [observability/gcp_monitoring.py](observability/gcp_monitoring.py); v1 relies on Cloud Run logs + log-based metrics.
...
- **Ingestion API (Phase 15):** [api/routes/ingest.py](api/routes/ingest.py) — `POST /ingest` `{ "source": "gcp_docs" }` returns `{ "status": "started", "job_id" }`; poll `GET /ingest/{job_id}` for `status` and `docs_indexed`. Set **`INGESTION_GCP_DOCS_DIR`** to a corpus directory (see [.env.example](.env.example)); requires **`GOOGLE_CLOUD_PROJECT`** and Qdrant. Job state is in-process (single worker); production should use a persisted store + Cloud Run Job per spec.
