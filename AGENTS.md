...
- **Local stack without Cloud Run:** [docs/local_dev.md](docs/local_dev.md) (`scripts/prepare_local.sh`, demo [demo/querymesh-demo.html](demo/querymesh-demo.html), optional Next.js UI [web/README.md](web/README.md)).
...
- **Auth:** set `API_KEY_PEPPER` in `.env`. Mint a key: `PYTHONPATH=. uv run python scripts/mint_api_key.py` (prints raw key once; digest is **HMAC-SHA256(api_key, pepper)** per spec §8). `POST /query` requires `Authorization: Bearer <raw_key>`. Optional **`PORTAL_JWT_SECRET`**: `POST /account/register` / `login` (portal JWT), then `POST /account/api-keys` to mint keys — see [docs/local_dev.md](docs/local_dev.md).
- **Sessions (Redis):** `REDIS_URL` required for `/query` and for [Settings](api/settings.py) (mint script included). Envelope key `querymesh:session:{session_id}`, TTL **24h**; omit `session_id` in the body to mint; wrong/unknown session → **403** `invalid_session` (spec §8). `thread_id` for LangGraph is `{user_internal_id}:{session_id}` (spec §7). **Multi-turn context:** checkpointed `messages` in [graph/pipeline.py](graph/pipeline.py) (``HumanMessage`` / ``AIMessage`` with ``add_messages``); Redis holds the **session envelope only**. Longer recall uses Postgres `user_memory` when the synthesizer calls `save_memory`.
...
- **Code agent (Phase 12):** [agents/code_agent.py](agents/code_agent.py) is the **only** caller of [tools/code_exec_tool.py](tools/code_exec_tool.py). E2B: `E2B_API_KEY`; optional `E2B_TEMPLATE_ID` (omit to use SDK default sandbox; set after building [e2b/Dockerfile](e2b/Dockerfile) for `google-cloud-*` on the image—see [E2B template docs](https://e2b.dev/docs/template/quickstart)). Sandboxes use `allow_internet_access=False`, no GCP credentials in env. Defaults: 15s command timeout, 64KiB combined stdout/stderr cap, 2 concurrent runs per process (`api/settings.py`).
...
- **Eval harness (Phase 17):** [evals/golden_dataset.json](evals/golden_dataset.json) + [evals/golden_loader.py](evals/golden_loader.py). **RAGAS:** `uv sync --group eval`; default `python -m evals.ragas_eval` validates golden data only (no LLM); with **`RUN_EVAL=1`**, **`GOOGLE_CLOUD_PROJECT`**, and ADC, runs faithfulness / answer relevancy / context precision & recall on retrieval rows (Vertex via `langchain-google-vertexai`; model from **`VERTEX_LLM_MODEL`** / env). Scores upload automatically to Langfuse (per-sample child spans + full pipeline config) when `LANGFUSE_PUBLIC_KEY` / `LANGFUSE_SECRET_KEY` are set. PR-style **`pytest`** excludes tests marked **`eval`** (see [pyproject.toml](pyproject.toml)); [tests/test_golden_loader_unit.py](tests/test_golden_loader_unit.py) stays in the fast suite.
