...
- **Langfuse (Phase 13+):** Set `LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, optional `LANGFUSE_HOST` in [.env.example](.env.example) / [api/settings.py](api/settings.py). Traces use the LangChain `CallbackHandler` on each `/query` graph run; response `trace_id` is the root trace id (UUID hex); `flush()` runs after each request for serverless-friendly export.
