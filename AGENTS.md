...
- **LangGraph:** Compiled pipeline in [graph/pipeline.py](graph/pipeline.py) (`echo` → `orchestrator` → `retrieve` → `rag_structured` → `synthesizer`). [memory/checkpointer.py](memory/checkpointer.py) uses Postgres via `AsyncPostgresSaver` (DSN: `postgresql+asyncpg://` → `postgresql://` for psycopg); `get_compiled_query_graph()` is the app singleton; [api/routes/query.py](api/routes/query.py) passes `configurable.thread_id`. App [lifespan](api/main.py) shutdown disposes the compiled graph, then the checkpoint pool, then SQLAlchemy and Redis.
