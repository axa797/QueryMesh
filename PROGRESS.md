...
- **17. Eval harness** — Done: [evals/golden_dataset.json](evals/golden_dataset.json) (30 rows), [evals/golden_loader.py](evals/golden_loader.py), [evals/ragas_eval.py](evals/ragas_eval.py); `eval` pytest marker excluded from default runs; `RUN_EVAL=1` + `uv sync --group eval` for LLM judges.
...
- **Phase 17:** [evals/](evals/) golden JSON + RAGAS runner; `eval` pytest marker; fast [tests/test_golden_loader_unit.py](tests/test_golden_loader_unit.py).
